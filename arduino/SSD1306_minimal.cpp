...
void SSD1306_Mini::printChar2( char ch, char row ){
    ...
    temp= getFlash(BasicFont, i*5 + 0);
    out= 0x0;
    out|=temp&(0x08<<(row*4))?0xc0:0;
    out|=temp&(0x04<<(row*4))?0x30:0;
    out|=temp&(0x02<<(row*4))?0x0c:0;
    out|=temp&(0x01<<(row*4))?0x03:0;
    data[0]= out;
    data[1]= out;

    temp= getFlash(BasicFont, i*5 + 1);
    out= 0x0;
    out|=temp&(0x08<<(row*4))?0xc0:0;
    out|=temp&(0x04<<(row*4))?0x30:0;
    out|=temp&(0x02<<(row*4))?0x0c:0;
...
void SSD1306_Mini::printString2( char col, char row, char * pText ){
  unsigned char i;
  unsigned char len = strlen( pText );
  
  // reserve space for font ( 2x (5cols) + 2cols (spaces) )
  clipArea( col, row, len * 7 * 2, 2 );
  
  // print upper half 
  for (i=0;i<len;i++){
    printChar2( pText[i], 0 );
  }
  // print lower half 
  for (i=0;i<len;i++){
    printChar2( pText[i], 1 );
  }
