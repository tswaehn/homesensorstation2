...
String LaCrosse::BuildDataString(struct Frame *frame) {
  ...
  pBuf += (byte) frame->Temperature;
  pBuf += ' ';
  
  pBuf += (byte) frame->Humidity;
  pBuf += ' ';
...
String LaCrosse::GetFhemDataString(byte *data) {
  ...
  if ((data[0] & 0xF0) >> 4 == 9) {
    ...
    DecodeFrame(data, &frame);
    if (frame.IsValid) {
      fhemString = BuildDataString(&frame);
      //
      registerSensorData( frame.ID, frame.Temperature, frame.Humidity, frame.WeakBatteryFlag );
    }
  }
